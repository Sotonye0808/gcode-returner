...
attribute easier.
"""
import py_svg2gcode.local_lib.cubicsuperpath as cubicsuperpath
import py_svg2gcode.local_lib.bezmisc as bezmisc
import copy, math, re
...
def composeParents(node, mat):
    ...
    if node.getparent().tag == index.addNS("g", "svg"):
...
def computeBBox(aList, mat=[[1, 0, 0], [0, 1, 0]]):
    ...
    for node in aList:
        ...
        if node.get("d"):
            ...
            index.addNS("rect", "svg"),
            "rect",
            index.addNS("image", "svg"),
            "image",
        ]:
            d = (
                "M"
        ...
        elif node.tag in [index.addNS("line", "svg"), "line"]:
            d = (
                "M"
                + node.get("x1")
                + ","
            ...
            index.addNS("circle", "svg"),
            "circle",
            index.addNS("ellipse", "svg"),
            "ellipse",
        ]:
            rx = node.get("r")
            if rx is not None:
        ...
        elif node.tag == index.addNS("use", "svg") or node.tag == "use":
            refid = node.get(index.addNS("href", "xlink"))
            path = '//*[@id="%s"]' % refid[1:]
